classdef StringValidator < BaseValidator
    ...
    methods
        ...
        function setRangeSpecialCase(self,rangeString)
            ...
            switch upper(rangeString)
                ...
                case '$MALEPARENT'
                    maleParentNames = StringValidator.maleParentNamesCache('get');
                    if isempty(maleParentNames)
                        self.setMaleParentNames();
                        maleParentNames = StringValidator.maleParentNamesCache('get');
                    end
                    self.allowedStrings = maleParentNames;       
                    
                case '$FEMALEPARENT'
                    femaleParentNames = StringValidator.femaleParentNamesCache('get');
                    if isempty(femaleParentNames)
                        self.setFemaleParentNames();
                        femaleParentNames = StringValidator.femaleParentNamesCache('get');
                    end
                    self.allowedStrings = femaleParentNames;       
                 
                case '$ODORS'
                    odorNames = StringValidator.odorNamesCache('get');
                    if isempty(odorNames)
                        self.setOdorNames();
                        odorNames = StringValidator.odorNamesCache('get');
                    end
                    self.allowedStrings = odorNames;
        ...
        function setMaleParentNames(self)
            cacher = SAGEDataCacher();
            try
                maleParentNames = cacher.readMaleParentFile();     
            catch ME
                waring( ...
                    'StringValidator unable to read effectors file: %s', ...
                    ME.message ...
                    );
                maleParentNames = dummyGetLineNames();
            end   
            StringValidator.maleParentNamesCache('set', maleParentNames);
        end 
        
        function setFemaleParentNames(self)
            cacher = SAGEDataCacher();
            try
                femaleParentNames = cacher.readFemaleParentFile();     
            catch ME
                waring( ...
                    'StringValidator unable to read effectors file: %s', ...
                    ME.message ...
                    );
                femaleParentNames = dummyGetLineNames();
            end   
            StringValidator.femaleParentNamesCache('set', femaleParentNames);
        end 

        function setOdorNames(self)
            cacher = SAGEDataCacher();
            try
                odorNames = cacher.readOdorFile();     
            catch ME
                waring( ...
                    'StringValidator unable to read effectors file: %s', ...
                    ME.message ...
                    );
                odorNames = dummyGetLineNames();
            end   
            StringValidator.odorNamesCache('set', odorNames);
        end        
    ...
    methods (Static)
        ...
        function rval = maleParentNamesCache(cmd, newMaleParentNames)
            % Provides a cache for the effector names
            persistent maleParentNames;
            rval = [];
            switch lower(cmd)
                case 'get'
                    rval = maleParentNames;
                case 'set'
                    maleParentNames = newMaleParentNames;
            end
        end
        
        function rval = femaleParentNamesCache(cmd, newFemaleParentNames)
            % Provides a cache for the effector names
            persistent femaleParentNames;
            rval = [];
            switch lower(cmd)
                case 'get'
                    rval = femaleParentNames;
                case 'set'
                    femaleParentNames = newFemaleParentNames;
            end
        end

        function rval = odorNamesCache(cmd, newOdorNames)
            % Provides a cache for the effector names
            persistent odorNames;
            rval = [];
            switch lower(cmd)
                case 'get'
                    rval = odorNames;
                case 'set'
                    odorNames = newOdorNames;
            end
        end        
