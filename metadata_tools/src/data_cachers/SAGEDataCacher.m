classdef SAGEDataCacher < handle
    ...
    properties (Constant)
        ...
        maleParentFile = 'maleParent.txt';
        femaleParentFile = 'femaleParent.txt';
        odorFile = 'odors.txt'
    ...
    methods
        ...
        function maleParentNames = readMaleParentFile(self)
            % Reads effector names file in the data directory
            filePath = self.getMaleParentFilePath();
            maleParentNames = cellFromTextFile(filePath);
        end
                      
        function femaleParentNames = readFemaleParentFile(self)
           % Reads effector names file in the data directory
           filePath = self.getFemaleParentFilePath();
           femaleParentNames = cellFromTextFile(filePath);
        end          
        
        function odorNames = readOdorFile(self)
           % Reads effector names file in the data directory
           filePath = self.getOdorFilePath();
           odorNames = cellFromTextFile(filePath);
        end    
        ...
        function filePath = getMaleParentFilePath(self)
           % Get the full path to the effectors file in the data directory.
           % Note it is assumed that the data directoty is located two 
            % directories up from this file in the top level direcoty of 
            % this project.
            dirPath = self.getDataDirPath();
            filePath = [dirPath,self.maleParentFile];
        end
        
        function filePath = getFemaleParentFilePath(self)
           % Get the full path to the effectors file in the data directory.
           % Note it is assumed that the data directoty is located two 
            % directories up from this file in the top level direcoty of 
            % this project.
            dirPath = self.getDataDirPath();
            filePath = [dirPath,self.femaleParentFile];
        end        

        function filePath = getOdorFilePath(self)
           % Get the full path to the effectors file in the data directory.
           % Note it is assumed that the data directoty is located two 
            % directories up from this file in the top level direcoty of 
            % this project.
            dirPath = self.getDataDirPath();
            filePath = [dirPath,self.odorFile];
        end           
